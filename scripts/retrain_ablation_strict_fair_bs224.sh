...
echo "[3/4] Cross C: PCD + MDIT (bs64, no-AMP)"
HF_HUB_OFFLINE=1 python scripts/run_mdit_autoresearch_trial.py \
  --phase full \
  ...
  --eval-episodes 20 \
  --device cuda \
  --experiment-name ablation_cross_pcd_mdit_bs64_noamp_eval50_100 \
  --run-name ablation_pcd_mdit_100_fair_bs64_noamp \
  --description "strict fair cross C: pcd+mdit, obs2/h100/a24, bs64, no-amp, eval@50/100" \
  --headless \
  --show-progress \
  --set use_amp=false \
  --set batch_size=64 \
  --cleanup-failed \
  2>&1 | tee logs/run_ablation_cross_pcd_mdit_bs64_noamp_eval50_100.log

echo "[4/4] Cross D: 5RGB + text + PDIT-adapter (MDIT line, bs64, no-AMP)"
HF_HUB_OFFLINE=1 python scripts/run_mdit_autoresearch_trial.py \
  --phase full \
  ...
  --eval-episodes 20 \
  --device cuda \
  --experiment-name ablation_cross_rgb5text_pdit_adapter_bs64_noamp_eval50_100 \
  --run-name ablation_rgb_text_pdit_100_fair_bs64_noamp \
  --description "strict fair cross D: 5rgb+text+pdit-adapter, obs2/h100/a24, bs64, no-amp, eval@50/100" \
  --headless \
  --show-progress \
  --set use_amp=false \
  --set batch_size=64 \
  --set observation_encoder.vision.train_mode=last_block \
  --cleanup-failed \
  2>&1 | tee logs/run_ablation_cross_rgb5text_pdit_adapter_bs64_noamp_eval50_100.log

echo "done: strict fair ablation (4 arms, bs64, no-amp, eval@50/100) finished."
