...
if [ -f "$SUCCESS_HIST" ]; then
    log "Success eval history found"
    python3 -c "
import json
...
" | tee -a "$LOG"
    # Proceed to gate decision below
...
if [ -f "$SUMMARY" ]; then
    COMPLETED=$(python3 -c "import json; d=json.load(open('$SUMMARY')); print(d.get('latest_epoch', 0))" 2>/dev/null || echo "0")
    log "Completed epochs: $COMPLETED"

    if [ "$COMPLETED" -ge 99 ] && [ ! -f "$SUCCESS_HIST" ]; then
