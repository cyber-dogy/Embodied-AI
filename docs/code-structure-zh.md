...
- `encoders/`
  ...
  - `encoders/dummy.py` 用于快速验证“替换一个 encoder 文件就能走通整条链路”
- `backbones/`
  - 轨迹主干网络
...
- `modalities/`
  ...
  - `modalities/dummy.py` 用于 replaceability smoke test
  - `modalities/rgb.py` 现在保留为清晰的预留入口，后续补真实 RGB pipeline 时就接在这里
- `replay_buffer.py`
  - 数据存储入口
- `sequence_sampler.py`
  - 时序切片采样
...
### 想换 `pcd -> rgb`
- 在 `src/.../data/modalities/` 增加真实 `rgb.py`
- 增加对应 encoder
