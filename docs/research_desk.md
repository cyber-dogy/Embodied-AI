...
MDIT 当前唯一经过共享审计链确认的锚点仍是 `0.55@100` 的 RGB+文本主线。稳定化对照弱于主线，faithful 对照的首轮失败被确认是缓存和网络问题，严格 MTDP 对照没有通过共享闸门；现阶段最重要的动作不是再开更多相似 run，而是把当前最佳路线从 `100 epoch` 稳定续到 `500 epoch` 并完成同口径审计。
