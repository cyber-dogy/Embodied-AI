...
def _unpack_normed_batch(normed_batch):
    if len(normed_batch) == 4:
        obs, robot_state_obs, robot_state_pred, task_text = normed_batch
    elif len(normed_batch) == 3:
        obs, robot_state_obs, robot_state_pred = normed_batch
        task_text = None
    else:
        raise ValueError(f"Unexpected normalized batch structure len={len(normed_batch)}")
    ...
    with torch.inference_mode():
        for batch_idx, batch_cpu in enumerate(loader):
            ...
            with get_autocast_context(cfg.train_use_amp, cfg.train_amp_dtype):
                ...
                normed_batch = model._norm_data(batch)
                obs, robot_state_obs, robot_state_pred, task_text = _unpack_normed_batch(normed_batch)
                pred_y = model.infer_y(obs, robot_state_obs, task_text=task_text)
                metrics = {
