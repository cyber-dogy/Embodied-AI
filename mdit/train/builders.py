...
from mdit.data import build_dataset
from mdit.model.model import MultiTaskDiTPolicy
from mdit.model.utils import move_to_device


def build_datasets(cfg: MDITExperimentConfig):
...
def build_policy(cfg: MDITExperimentConfig, dataset_stats: dict[str, Any]) -> MultiTaskDiTPolicy:
    device = set_device(cfg.device)
    policy = MultiTaskDiTPolicy(cfg, dataset_stats)
    policy.to(device)
    return policy


def build_optimizer(policy: MultiTaskDiTPolicy, cfg: MDITExperimentConfig) -> torch.optim.Optimizer:
...
def build_scheduler(optimizer: torch.optim.Optimizer, cfg: MDITExperimentConfig, train_loader_len: int):
