...
EVAL_CONTRACT_KEYS = (
    ...
    "vision_lr_multiplier",
    "vision_encode_chunk_size",
    ...
    "fm_exp_scale",
    "fm_sigma_min",
    "fm_num_integration_steps",
    "fm_integration_method",
    "fm_timestep_sampling_strategy",
    "fm_timestep_beta_alpha",
    "fm_timestep_beta_beta",
    "fm_timestep_beta_s",
    "command_mode",
    "horizon_index",
    "average_first_n",
...
def build_eval_contract(
    ...
    *,
    ...
    contract = {
        ...
        "fm_variant": str(payload.get("fm_variant") or "standard"),
        "normalization_profile": str(payload.get("normalization_profile") or "legacy"),
        ...
        "vision_lr_multiplier": float(payload.get("vision_lr_multiplier") or 1.0),
        "vision_encode_chunk_size": int(payload.get("vision_encode_chunk_size") or 0),
        "activation_checkpointing": bool(payload.get("activation_checkpointing") or False),
        ...
        "use_rope": bool(payload.get("use_rope") or False),
        "rope_base": float(payload.get("rope_base") or 10000.0),
        "use_positional_encoding": bool(payload.get("use_positional_encoding", True)),
        "robot_state_mean": payload.get("robot_state_mean"),
        "robot_state_std": payload.get("robot_state_std"),
        "state_min": payload.get("state_min"),
        "state_max": payload.get("state_max"),
        "action_min": payload.get("action_min"),
        "action_max": payload.get("action_max"),
        "fm_num_k_infer": int(payload["fm_num_k_infer"]),
        "fm_flow_schedule": str(payload["fm_flow_schedule"]),
        "fm_exp_scale": None if payload.get("fm_exp_scale") is None else float(payload["fm_exp_scale"]),
        "fm_sigma_min": float(payload.get("fm_sigma_min") or 0.0),
        "fm_num_integration_steps": int(payload.get("fm_num_integration_steps") or 100),
        "fm_integration_method": str(payload.get("fm_integration_method") or "euler"),
        "fm_timestep_sampling_strategy": str(
            payload.get("fm_timestep_sampling_strategy") or "uniform"
        ),
        "fm_timestep_beta_alpha": float(payload.get("fm_timestep_beta_alpha") or 1.5),
        "fm_timestep_beta_beta": float(payload.get("fm_timestep_beta_beta") or 1.0),
        "fm_timestep_beta_s": float(payload.get("fm_timestep_beta_s") or 0.999),
