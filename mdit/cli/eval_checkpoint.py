...
def _looks_like_pdit_checkpoint(payload: dict[str, Any]) -> bool:
    cfg = payload.get("cfg")
    if isinstance(cfg, dict):
        if str(cfg.get("obs_mode", "")).lower() == "pcd":
            return True
        if str(cfg.get("encoder_name", "")).lower() == "pointnet_token":
            return True
    model_state = payload.get("model_state_dict")
    if isinstance(model_state, dict):
        keys = list(model_state.keys())
        if any("pointnet" in key.lower() for key in keys):
            return True
    return False
...
def main() -> int:
    ...
    if _looks_like_pdit_checkpoint(payload):
        raise SystemExit(
            "The checkpoint appears to be a PDIT checkpoint, not an MDIT checkpoint. "
            "Please run scripts/eval_pdit_checkpoint.py for this file."
        )
