...
TRIAL_REQUEST_FILENAME = "lelan_trial_request.json"
EXPERIMENT_MANIFEST_FILENAME = "experiment_manifest.json"


@dataclass(slots=True)
class LeLaNTrialRequest:
    config_path: Path
    ...
    experiment_name: str = "lelan_trial"
    description: str = ""
    max_steps: int = 200
...
def _trial_request_to_dict(request: LeLaNTrialRequest) -> dict[str, Any]:
...
def _trial_request_from_dict(payload: dict[str, Any]) -> LeLaNTrialRequest:
    ...
    return LeLaNTrialRequest(**payload_copy)


def _write_trial_request(run_dir: Path, request: LeLaNTrialRequest) -> Path:
...
def _config_diff(base_cfg: LeLaNExperimentConfig, resolved_cfg: LeLaNExperimentConfig) -> dict[str, dict[str, Any]]:
...
def _build_change_summary(base_cfg: LeLaNExperimentConfig, resolved_cfg: LeLaNExperimentConfig) -> list[str]:
    summary: list[str] = []
    ...
    if float(base_cfg.fusion_transformer.dropout) != float(resolved_cfg.fusion_transformer.dropout):
        summary.append(
            f"fusion_transformer.dropout: {base_cfg.fusion_transformer.dropout} -> "
            f"{resolved_cfg.fusion_transformer.dropout}"
        )
    if bool(base_cfg.smooth_actions) != bool(resolved_cfg.smooth_actions):
        summary.append(f"smooth_actions: {bool(base_cfg.smooth_actions)} -> {bool(resolved_cfg.smooth_actions)}")
    return summary
...
def _write_experiment_manifest(
    ...
    *,
    ...
    base_cfg: LeLaNExperimentConfig,
    resolved_cfg: LeLaNExperimentConfig,
    resolved_request: LeLaNTrialRequest,
) -> Path:
    payload = {
        "line": "lelan",
...
def _load_trial_request(run_dir: Path) -> LeLaNTrialRequest:
...
def _infer_legacy_trial_request(run_dir: Path) -> LeLaNTrialRequest:
    config_path = run_dir / "config.json"
    if not config_path.exists():
        raise FileNotFoundError(
            f"LeLaN trial request manifest not found: {_trial_request_path(run_dir)} "
    ...
    return LeLaNTrialRequest(
        config_path=PROJECT_ROOT / "configs" / "lelan" / "baseline.json",
...
def _build_offline_audit_command(run_dir: Path) -> str:
    cmd = [
        sys.executable,
        str(PROJECT_ROOT / "scripts" / "run_lelan_autoresearch_trial.py"),
        "--phase",
        "audit-only",
        "--run-dir",
        str(run_dir),
...
def _prepare_cfg(request: LeLaNTrialRequest) -> LeLaNExperimentConfig:
...
def _resolved_request(request: LeLaNTrialRequest, cfg: LeLaNExperimentConfig) -> LeLaNTrialRequest:
...
def _run_checkpoint_audit(
    *,
    ...
    cmd = [
        sys.executable,
        str(PROJECT_ROOT / "scripts" / "eval_lelan_all_checkpoints.py"),
        "--ckpt-epochs-dir",
        str(ckpt_dir),
        "--results-json",
        str(results_json),
...
def _build_drop_checks(records: list[dict[str, Any]], *, tolerance: float) -> list[dict[str, Any]]:
    checks: list[dict[str, Any]] = []
    best_so_far = None
    best_epoch = None
...
def _keep_epoch_paths(records: list[dict[str, Any]]) -> list[Path]:
    return [path for row in records if (path := Path(row["path"]).resolve()).exists()]
...
def _build_train_keep_paths(run_dir: Path, cfg: LeLaNExperimentConfig) -> list[Path]:
    keep_paths = [
        run_dir / "config.json",
        cfg.summary_path,
...
def train_lelan_autoresearch_trial(request: LeLaNTrialRequest, *, log_results: bool = True) -> dict[str, Any]:
    from lelan.train.runner import train_experiment
...
def finalize_lelan_autoresearch_trial(
    run_dir: str | Path,
    *,
    request_overrides: LeLaNTrialRequest | dict[str, Any] | None = None,
    ...
    if request_overrides is not None:
        ...
        override_payload = (
            ...
            if isinstance(request_overrides, LeLaNTrialRequest)
...
def run_lelan_autoresearch_trial(request: LeLaNTrialRequest, *, log_results: bool = True) -> dict[str, Any]:
    train_output = train_lelan_autoresearch_trial(request, log_results=log_results)
    if not train_output.get("pending_offline_audit"):
        return train_output
    return finalize_lelan_autoresearch_trial(
