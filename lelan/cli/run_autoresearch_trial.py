...
from research.lelan_trial_runner import LeLaNTrialRequest


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run one clean LeLaN trial in full, train-only, or audit-only mode."
    )
    parser.add_argument(
        "--phase",
        type=str,
    ...
    parser.add_argument(
        "--config",
        type=Path,
        default=PROJECT_ROOT / "configs" / "lelan" / "baseline.json",
    ...
    parser.add_argument("--experiment-name", type=str, default="lelan_trial", help="Short trial label for logs.")
    ...
    parser.add_argument(
        ...
        action=argparse.BooleanOptionalAction,
        default=None,
        help="Prefer EMA weights during offline evaluation (default: true).",
    )
    parser.add_argument(
        "--set",
...
def main() -> int:
    ...
    request = LeLaNTrialRequest(**request_kwargs)
    if args.phase == "full":
        result = run_lelan_autoresearch_trial(request)
    elif args.phase == "train-only":
        result = train_lelan_autoresearch_trial(request)
    else:
        audit_override_kwargs = {}
        for key, value in optional_fields.items():
            if value is not None:
                audit_override_kwargs[key] = value
        result = finalize_lelan_autoresearch_trial(
