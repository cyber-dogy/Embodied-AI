...
from lelan.config import LeLaNExperimentConfig, apply_config_overrides, load_config


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Train the LeLaN policy from a JSON config.")
    parser.add_argument(
        "--config",
        type=Path,
        default=PROJECT_ROOT / "configs" / "lelan" / "baseline.json",
        help="Path to a LeLaN training config JSON file.",
    )
    ...
    parser.add_argument(
        ...
        default=None,
        help="Enable wandb logging for this LeLaN training run.",
    )
...
def apply_train_overrides(cfg: LeLaNExperimentConfig, args: argparse.Namespace) -> LeLaNExperimentConfig:
    if args.run_name:
