...
def _run_eval_subprocess(
    ...
    *,
    ...
    with tempfile.TemporaryDirectory(prefix="lelan-eval-") as tmpdir:
        ...
        cmd = [
            sys.executable,
            str(PROJECT_ROOT / "scripts" / "eval_lelan_checkpoint.py"),
            "--ckpt-path",
            str(ckpt_path),
            "--episodes",
            str(int(episodes)),
...
def maybe_plot(results: dict[str, Any], plot_path: Path | None, figsize: tuple[float, float]) -> None:
    ...
    ax.set_title("LeLaN checkpoint success rate")
...
def main() -> int:
    ...
    iterator = tqdm(records, desc="lelan ckpts", leave=False) if tqdm is not None else records
