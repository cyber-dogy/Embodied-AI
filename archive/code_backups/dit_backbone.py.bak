...
class ShiftScaleMod(nn.Module):
    ...
    def forward(self, x: torch.Tensor, cond: torch.Tensor) -> torch.Tensor:
        cond = self.act(cond)
        return x * self.scale(cond)[None] + self.shift(cond)[None]
...
class DiTDecoderBlock(nn.Module):
    ...
    def reset_parameters(self) -> None:
        for p in self.parameters():
            if p.dim() > 1:
                nn.init.xavier_uniform_(p)
        for block in (self.attn_mod1, self.attn_mod2, self.mlp_mod1, self.mlp_mod2):
...
class DiTTrajectoryBackbone(nn.Module):
    def __init__(
        ...
        self.output_head = FinalLayer(hidden_dim, self.output_dim)
    ...
    def forward_dec(
        ...
        return self.output_head(dec_out, time_enc, enc_cache[-1])
